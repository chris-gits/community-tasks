...
def register_routes(app, database, bcrypt):
    ...
    def route_api_create_task():
        ...
        form = request.form
        form_is_valid = validate_form(form, "Could not create task since the following details are missing: ", {
            "title": "Title",
            "description": "Description",
            "deadline": "Deadline"
            })
        if form_is_valid:
            task_deadline_formatted = DateTime.datetime.fromisoformat(form.get("deadline"))
            if task_deadline_formatted <= DateTime.datetime.now():
                Flask.flash(
                    "Could not create task since deadline is set to the past.",
                    category="Danger"
                )
            else:
                try:
                    new_task = Models.tasks.Task(
                        ...
                        deadline = task_deadline_formatted
                    )
                    ...
                    Flask.flash("Could not create task due to internal error.", category="Danger")
    ...
    def route_api_claim_task():
        ...
        request = Flask.request
        form = request.form
        form_is_valid = validate_form(form, "Could not claim task since the following details are missing: ", {
            "id": "ID"
            })
        if form_is_valid:
            try:
    ...
    def route_api_drop_task():
        ...
        request = Flask.request
        form = request.form
        form_is_valid = validate_form(form, "Could not drop task since the following details are missing: ", {
            "id": "ID"
            })
        if form_is_valid:
            try:
    ...
    def route_api_complete_task():
        ...
        request = Flask.request
        form = request.form
        form_is_valid = validate_form(form, "Could not mark task as complete since the following details are missing: ", {
            "id": "ID"
        })
        if form_is_valid:
            try:
